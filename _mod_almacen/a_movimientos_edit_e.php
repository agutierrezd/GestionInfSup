...
$maxRows_rsultimonumero = 10;
$pageNum_rsultimonumero = 0;
if (isset($_GET['pageNum_rsultimonumero'])) {
  $pageNum_rsultimonumero = $_GET['pageNum_rsultimonumero'];
}
$startRow_rsultimonumero = $pageNum_rsultimonumero * $maxRows_rsultimonumero;

mysql_select_db($database_oConnAlmacen, $oConnAlmacen);
$query_rsultimonumero = "SELECT alasientos.ascodalmacen, Max(alasientos.as_nroasiento) AS Q, Max(alasientos.as_fechaasiento) AS FECHA FROM alasientos WHERE ascodalmacen <= '101' GROUP BY alasientos.ascodalmacen";
$query_limit_rsultimonumero = sprintf("%s LIMIT %d, %d", $query_rsultimonumero, $startRow_rsultimonumero, $maxRows_rsultimonumero);
$rsultimonumero = mysql_query($query_limit_rsultimonumero, $oConnAlmacen) or die(mysql_error());
$row_rsultimonumero = mysql_fetch_assoc($rsultimonumero);

if (isset($_GET['totalRows_rsultimonumero'])) {
  $totalRows_rsultimonumero = $_GET['totalRows_rsultimonumero'];
} else {
  $all_rsultimonumero = mysql_query($query_rsultimonumero);
  $totalRows_rsultimonumero = mysql_num_rows($all_rsultimonumero);
}
$totalPages_rsultimonumero = ceil($totalRows_rsultimonumero/$maxRows_rsultimonumero)-1;
...
<table width="800" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td><table width="410" border="0" cellspacing="2" cellpadding="0">
      <tr>
        <td class="frmtablahead">ALMACEN</td>
        <td class="frmtablahead">ULTIMO N&Uacute;MERO</td>
        <td class="frmtablahead">ULTIMA FECHA</td>
      </tr>
      <?php do { ?>
        <tr class="frmtablabody">
          <td class="titlemsg2"><?php echo $row_rsultimonumero['ascodalmacen']; ?></td>
          <td class="titlemsg2"><?php echo $row_rsultimonumero['Q']; ?></td>
          <td class="titlemsg2"><?php echo $row_rsultimonumero['FECHA']; ?></td>
        </tr>
        <?php } while ($row_rsultimonumero = mysql_fetch_assoc($rsultimonumero)); ?>
    </table></td>
  </tr>
  <tr>
    <td>&nbsp;
...
?>
                ...
                <tr>
                  ...
                  <td><input type="text" name="as_nroasiento_<?php echo $cnt1; ?>" id="as_nroasiento_<?php echo $cnt1; ?>" value="<?php echo KT_escapeAttribute($row_rsalasientos['as_nroasiento']); ?>" size="7" />
...
mysql_free_result($rsultimonumero);
?>
