...
$masterValidation->addField("do_file", true, "", "", "", "", "");
...
function Trigger_FileUpload(&$tNG) {
  ...
  $uploadObj->setFormFieldName("do_file");
  $uploadObj->setDbFieldName("do_file");
  $uploadObj->setFolder("../_attach_almacen/_docs_soporte/");
  $uploadObj->setMaxSize(10500);
  $uploadObj->setAllowedExtensions("pdf");
  $uploadObj->setRename("custom");
  $uploadObj->setRenameRule("{doclasedoc_id}_{alasiento_id_fk}_{doccnit}.{KT_ext}");
...
mysql_select_db($database_oConnAlmacen, $oConnAlmacen);
$query_rsclase = "SELECT * FROM geclasedocto WHERE cd_ctrl = 1";
...
mysql_select_db($database_oConnAlmacen, $oConnAlmacen);
$query_rsdependencias = "SELECT * FROM gedependencias WHERE de_ctrl = 1";
...
$ins_gedocumentos->registerTrigger("END", "Trigger_Default_Redirect", 99, "a_docs_soporte_list.php?as_id={GET.as_id}&anio_id={GET.anio_id}&hash_id={GET.hash_id}");
...
$ins_gedocumentos->registerTrigger("AFTER", "Trigger_FileUpload", 97);
// Add columns
...
$ins_gedocumentos->addColumn("do_file", "FILE_TYPE", "FILES", "do_file");
...
<form method="post" id="form1" action="<?php echo KT_escapeAttribute(KT_getFullUri()); ?>" enctype="multipart/form-data">
  <table cellpadding="2" cellspacing="0" class="KT_tngtable">
    <tr>
      <td class="KT_th"><label for="alasiento_id_fk"></label></td>
      <td><input name="alasiento_id_fk" type="hidden" id="alasiento_id_fk" value="<?php echo $_GET['as_id']; ?>" size="32" />
          <?php echo $tNGs->displayFieldHint("alasiento_id_fk");?> <?php echo $tNGs->displayFieldError("gedocumentos", "alasiento_id_fk"); ?> <input name="sys_user" type="hidden" id="sys_user" value="<?php echo $_SESSION['kt_login_user']; ?>" size="32" />
          <?php echo $tNGs->displayFieldHint("sys_user");?> <?php echo $tNGs->displayFieldError("gedocumentos", "sys_user"); ?> <input type="hidden" name="sys_fecha" id="sys_fecha" value="<?php echo $fechac; ?>" size="32" />
          <?php echo $tNGs->displayFieldHint("sys_fecha");?> <?php echo $tNGs->displayFieldError("gedocumentos", "sys_fecha"); ?> </td>
    </tr>
    <tr>
      <td class="KT_th"><label for="doclasedoc">Clase:</label></td>
      <td><select name="doclasedoc" id="doclasedoc">
        <option value="" <?php if (!(strcmp("", $row_rsgedocumentos['doclasedoc']))) {echo "selected=\"selected\"";} ?>>Seleccione..</option>
        <?php
do {  
?><option value="<?php echo $row_rsclase['cd_clasedoc']?>"<?php if (!(strcmp($row_rsclase['cd_clasedoc'], $row_rsgedocumentos['doclasedoc']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsclase['cd_nombredoc']?></option>
          <?php
...
?>
    ...
    <tr>
      <td class="KT_th"><label for="do_nrodoc">N&uacute;mero:</label></td>
      <td><input name="do_nrodoc" type="text" id="do_nrodoc" value="<?php echo $row_rsconsecutivo['qtyconsecutivo'] + 1; ?>" size="32" />
    ...
    <tr>
      ...
      <td><input name="do_fechadoc" type="text" id="do_fechadoc" value="<?php echo $row_rsinfoasiento['as_fechaasiento']; ?>" size="32" readonly="true" />
          <?php echo $tNGs->displayFieldHint("do_fechadoc");?> <?php echo $tNGs->displayFieldError("gedocumentos", "do_fechadoc"); ?> </td>
    </tr>
    <tr>
      <td class="KT_th">&nbsp;</td>
      <td><input type="hidden" name="docodregion" id="docodregion" value="<?php echo $row_rsinfoasiento['ascodalmacen']; ?>" size="32" />
          <?php echo $tNGs->displayFieldHint("docodregion");?> <?php echo $tNGs->displayFieldError("gedocumentos", "docodregion"); ?> </td>
    </tr>
    <tr>
      <td class="KT_th"><label for="do_detalle">Detalle:</label></td>
      <td><textarea name="do_detalle" id="do_detalle" cols="50" rows="3"><?php echo KT_escapeAttribute($row_rsgedocumentos['do_detalle']); ?></textarea>
...
?>
    ...
    <tr>
      <td class="KT_th"><label for="doccnit">N&uacute;mero documento:</label></td>
      <td><input type="text" name="doccnit" id="doccnit" value="<?php echo KT_escapeAttribute($row_rsgedocumentos['doccnit']); ?>" size="32" />
          <?php echo $tNGs->displayFieldHint("doccnit");?> <?php echo $tNGs->displayFieldError("gedocumentos", "doccnit"); ?> </td>
    </tr>
    <tr>
      <td class="KT_th"><label for="do_file">Anexo:</label></td>
      <td><input type="file" name="do_file" id="do_file" size="32" />
          <?php echo $tNGs->displayFieldError("gedocumentos", "do_file"); ?> </td>
    </tr>
    <tr class="KT_buttons">
      <td colspan="2"><hr />      </td>
    </tr>
    <tr>
      <td class="KT_th">&nbsp;</td>
      <td><input name="mdclasedoc" type="hidden" id="mdclasedoc" value="<?php echo KT_escapeAttribute($row_rsalmovtodia['mdclasedoc']); ?>" wdg:subtype="N1DependentField" wdg:type="widget" wdg:recordset="rsclase" wdg:valuefield="cd_clasedoc" wdg:pkey="cd_clasedoc" wdg:triggerobject="doclasedoc" />
          <?php echo $tNGs->displayFieldHint("mdclasedoc");?> <?php echo $tNGs->displayFieldError("almovtodia", "mdclasedoc"); ?> <input type="hidden" name="mdnrodoc" id="mdnrodoc" value="<?php echo $row_rsconsecutivo['qtyconsecutivo'] + 1; ?>" size="32" />
          <?php echo $tNGs->displayFieldHint("mdnrodoc");?> <?php echo $tNGs->displayFieldError("almovtodia", "mdnrodoc"); ?> <input type="hidden" name="mdfechadoc" id="mdfechadoc" value="<?php echo $row_rsinfoasiento['as_fechaasiento']; ?>" size="32" />
          <?php echo $tNGs->displayFieldHint("mdfechadoc");?> <?php echo $tNGs->displayFieldError("almovtodia", "mdfechadoc"); ?> <input type="hidden" name="mdalmacen" id="mdalmacen" value="<?php echo $row_rsinfoasiento['ascodalmacen']; ?>" size="32" />
          <?php echo $tNGs->displayFieldHint("mdalmacen");?> <?php echo $tNGs->displayFieldError("almovtodia", "mdalmacen"); ?> <input type="hidden" name="md_legalizado" id="md_legalizado" value="N" size="32" />
          <?php echo $tNGs->displayFieldHint("md_legalizado");?> <?php echo $tNGs->displayFieldError("almovtodia", "md_legalizado"); ?> <input type="hidden" name="mdtipoestruc" id="mdtipoestruc" value="V" size="32" />
          <?php echo $tNGs->displayFieldHint("mdtipoestruc");?> <?php echo $tNGs->displayFieldError("almovtodia", "mdtipoestruc"); ?> <input type="hidden" name="mdnroasiento" id="mdnroasiento" value="<?php echo $row_rsinfoasiento['as_nroasiento']; ?>" size="32" />
...
?>
    ...
    <tr>
      <td class="KT_th">&nbsp;</td>
      <td><input name="geusuario" type="hidden" id="geusuario" value="<?php echo $_SESSION['kt_login_user']; ?>" size="32" />
          ...
          <?php echo $tNGs->displayFieldHint("gefechaactua");?> <?php echo $tNGs->displayFieldError("almovtodia", "gefechaactua"); ?> </td>
    </tr>
