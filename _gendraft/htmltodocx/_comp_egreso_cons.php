...
$colname_rselementossum = "-1";
if (isset($_GET['doclasedoc_id'])) {
  $colname_rselementossum = $_GET['doclasedoc_id'];
}
mysql_select_db($database_oConnAlmacen, $oConnAlmacen);
$query_rselementossum = sprintf("SELECT * FROM q_almovconsdia_egreso_asignado_sum WHERE sys_doclasedoc_id_fk = %s", GetSQLValueString($colname_rselementossum, "int"));
$rselementossum = mysql_query($query_rselementossum, $oConnAlmacen) or die(mysql_error());
$row_rselementossum = mysql_fetch_assoc($rselementossum);
$totalRows_rselementossum = mysql_num_rows($rselementossum);

$colname_rselementoslist = "-1";
if (isset($_GET['doclasedoc_id'])) {
  $colname_rselementoslist = $_GET['doclasedoc_id'];
}
mysql_select_db($database_oConnAlmacen, $oConnAlmacen);
$query_rselementoslist = sprintf("SELECT * FROM q_almovconsdia_egreso_asignado_global WHERE sys_doclasedoc_id_fk = %s", GetSQLValueString($colname_rselementoslist, "int"));
$rselementoslist = mysql_query($query_rselementoslist, $oConnAlmacen) or die(mysql_error());
//$row_rselementoslist = mysql_fetch_assoc($rselementoslist);
$totalRows_rselementoslist = mysql_num_rows($rselementoslist);

$colname_rselementosgroup = "-1";
if (isset($_GET['doclasedoc_id'])) {
  $colname_rselementosgroup = $_GET['doclasedoc_id'];
}
mysql_select_db($database_oConnAlmacen, $oConnAlmacen);
$query_rselementosgroup = sprintf("SELECT * FROM q_almovconsdia_egreso_asignado_group WHERE sys_doclasedoc_id_fk = %s", GetSQLValueString($colname_rselementosgroup, "int"));
$rselementosgroup = mysql_query($query_rselementosgroup, $oConnAlmacen) or die(mysql_error());
//$row_rselementosgroup = mysql_fetch_assoc($rselementosgroup);
$totalRows_rselementosgroup = mysql_num_rows($rselementosgroup);
?>
...
$texto3 = utf8_encode("FECHA DEL COMPROBANTE:<strong> ".$row_rsalmovtodia['do_fechadoc']."</strong>       NÚMERO DE COMPROBANTE:<strong> ".$row_rsgedocumentos['do_nrodoc']."</strong><br />");
$texto3a = utf8_encode("ASIENTO:<strong> ".$row_rsalmovtodia['mdnroasiento']."</strong>       FECHA DEL ASIENTO:<strong> ".$row_rsalmovtodia['mdfechadoc']."</strong><br />");
...
$total = $row_rselementossum['VALORTOTALG'];
$V=new EnLetras();
...
$texto14 = utf8_encode("<br /><br />ALMACENISTA:<strong> HERIBERTO CASTRILL&Oacute;N SANCHEZ</strong>");
$texto15 = utf8_encode("<br />C.C. 4979088<br />");
$texto16 = utf8_encode("<br /><br />FIRMA FUNCIONARIO / CONTRATISTA QUE RECIBE:<br /><strong>".$row_rsinfousuario['func_nombres']."</strong><br />CC:&nbsp;".$row_rsgedocumentos['doccnit']."<br />");
...
$html_dom->load('<html><body>'.$texto2.$texto3.$texto3a.$texto4.$texto5.$texto7.$texto6.$texto8.$texto9.$texto10.'</body></html>');
//Fin Objeto HTML
...
$table->addCell(4500)->addImage('sns_salida.png');
...
$table->addCell(4500)->addImage('foot.png');
...
$table->addCell(2500, $styleCell)->addText('PRESENTACION', $fontStyle);
$table->addCell(2500, $styleCell)->addText('CANTIDAD', $fontStyle);
$table->addCell(2500, $styleCell)->addText('VR UNITARIO', $fontStyle);
$table->addCell(2500, $styleCell)->addText('VALOR TOTAL', $fontStyle);


//for($i = 1; $i <= 10; $i++) {
while ($row_rselementoslist = mysql_fetch_array($rselementoslist)){
	$table->addRow();
	$table->addCell(2500)->addText(utf8_encode($row_rselementoslist['mcdcuenta']));
	$table->addCell(2500)->addText(utf8_encode($row_rselementoslist['mcdcodelem']));
	$table->addCell(2500)->addText(utf8_encode($row_rselementoslist['ec_nomelemento']));
	$table->addCell(2500)->addText(utf8_encode($row_rselementoslist['um_nomunimed']));
	$table->addCell(2500)->addText(utf8_encode($row_rselementoslist['QSOLICITADO']));
	$table->addCell(2500)->addText(number_format($row_rselementoslist['VRUNITARIO'],2,",","."));
	$table->addCell(2500)->addText(number_format($row_rselementoslist['VRTOTAL'],2,",","."));
//}
}
...
//for($i = 1; $i <= 10; $i++) {
while ($row_rselementosgroup = mysql_fetch_array($rselementosgroup)){
	$table->addRow();
	$table->addCell(2500)->addText(utf8_encode($row_rselementosgroup['CUENTA']));
	$table->addCell(2500)->addText(utf8_encode($row_rselementosgroup['NOMBRE']));
	$table->addCell(2500)->addText(utf8_encode($row_rselementosgroup['QCANTIDAD']));
	$table->addCell(2500)->addText(utf8_encode(number_format($row_rselementosgroup['VALORTOTAL'],2,',','.')));
//}
}
...
$html_dom2->load('<html><body>'.$texto14.$texto15.$texto16.'</body></html>');
...
mysql_free_result($rselementossum);

mysql_free_result($rselementoslist);

mysql_free_result($rselementosgroup);
?>