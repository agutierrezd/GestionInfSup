...
mysql_select_db($database_oConnContratos, $oConnContratos);
$query_rspagos = sprintf("SELECT * FROM contrato_controlpagos WHERE id_cont_fk = %s", GetSQLValueString($colname_rspagos, "int"));
...
$texto0 = utf8_encode("INFORME No.: ");
$texto0a = utf8_encode($row_rsinformesreg['inf_consecutivo']."<br />");
$texto1 = utf8_encode("FECHA EN QUE SE RINDE EL INFORME: ");
$texto1a = utf8_encode($row_rsinformesreg['inf_fechapresenta']."<br />");
$texto2 = utf8_encode("PERIODICIDAD: ");
$texto2a = utf8_encode($row_rsinfodash['periodo_name']."<br />");
$texto3 = utf8_encode("PERIODO REPORTADO: (DESDE: ");
$texto3a = utf8_encode($row_rsinformesreg['inf_fecharep_i']);
$texto3b = utf8_encode(" HASTA: ");
$texto3c = utf8_encode($row_rsinformesreg['inf_fecharep_f'].")<br />");
$texto4 = utf8_encode("<em>El presente informe se presenta en cumplimiento de lo dispuesto en la Resolución No. 5352 del 30 de diciembre de 2011 Por la cual se adopta el manual de Contratación, supervisión o interventoría del Ministerio de Comercio Industria y Turismo y se dictan otras disposiciones.</em>");
...
$texto11 = utf8_encode("Proyecto de inversión: ".$row_rsinformesreg['inf_rubrocode']." <br /> ");
$texto12 = utf8_encode("Objeto del contrato:<br />");
...
$texto16 = utf8_encode("Supervisor: ".$row_rsinformesreg['inf_nombre']." <br /> ");
$texto17 = utf8_encode("Cargo: ".$row_rsinformesreg['inf_cargo']." Dependencia: ".$row_rsinformesreg['inf_dependencia']." <br /> ");
$texto18 = utf8_encode("INFORMACIÓN FINANCIERA:");
if ($totalRows_rsgarantias > 0) {$texto19 = utf8_encode("GARANTÍAS: <br />");}
...
$texto25 = utf8_encode($row_rsinformesreg['inf_nombre']." <br /> ");
$texto26 = utf8_encode($row_rsinformesreg['inf_cargo']." <br /> ");
if ($totalRows_rsinfanexos > 0) {$texto27 = utf8_encode("Anexos: <br />");}
...
$html_dom->load('<html><body>'.$texto0.$texto0a.$texto1.$texto1a.$texto2.$texto2a.$texto3.$texto3a.$texto3b.$texto3c.$texto4.$texto6.$texto7.$texto7a.$texto8.$texto8a.$texto9.$texto10.$texto11.$texto12.$texto12a.$texto13.$texto14.$texto15.$texto16.$texto17.$texto19.'</body></html>');
//Fin Objeto HTML
...
// INICIA CREACION DE TABLA GARANTIAS

if ($totalRows_rsgarantias > 0) { // Muestra la tabla si existen valores , sino la oculta 


$styleTablez = array('borderSize'=>6, 'borderColor'=>'000000', 'cellMargin'=>80);
$styleFirstRowz = array('borderBottomSize'=>18, 'borderBottomColor'=>'000000', 'bgColor'=>'F7F7F7');

// Define cell style arrays
$styleCellz = array('valign'=>'center');

// Define font style for first row
$fontStylez = array('bold'=>true, 'align'=>'center');

// Add table style
$phpword_object->addTableStyle('myOwnTableStylez', $styleTablez, $styleFirstRowz);

// Add table
$table = $section->addTable('myOwnTableStylez');

// Add row
$table->addRow(900);

// Add cells
$table->addCell(2500, $styleCellz)->addText('GARANTIA', $fontStylez);
$table->addCell(2500, $styleCellz)->addText('VALOR', $fontStylez);
$table->addCell(2500, $styleCellz)->addText('FECHA DESDE', $fontStylez);
$table->addCell(2500, $styleCellz)->addText('FECHA HASTA', $fontStylez);

while ($row_rsgarantias = mysql_fetch_array($rsgarantias)){
//for($i = 1; $i <= 10; $i++) {
	$table->addRow();
	$table->addCell(2500)->addText(utf8_encode($row_rsgarantias['poliza_type_name']));
	$table->addCell(2500)->addText(number_format($row_rsgarantias['poli_valor'],2,",","."));
	$table->addCell(2500)->addText($row_rsgarantias['poli_vigenciadesde']);
	$table->addCell(2500)->addText($row_rsgarantias['poli_vigenciahasta']);
}
}

//FIN DE TABLA GARANTIAS

//************************************	

// INICIA CREACION DE TABLA 
$styleTablex = array('borderSize'=>6, 'borderColor'=>'006699');
$styleFirstRowx = array('borderBottomSize'=>18, 'borderBottomColor'=>'0000FF', 'bgColor'=>'66BBFF');

// Define cell style arrays
$styleCellx = array('valign'=>'center');

// Define font style for first row
$fontStylex = array('bold'=>false, 'align'=>'center');

// Add table style
$phpword_object->addTableStyle('myOwnTableStylez', $styleTablex, $styleFirstRowx);

// Add table
$table = $section->addTable('myOwnTableStylex');

// Add row
$table->addRow(900);

// Add cells
$table->addCell(9000, $styleCellx)->addText($texto89, $fontStylex);

$table->addRow(900);
$table->addCell(9000, $styleCellx)->addText($avgejec, $fontStylex);

$table->addRow(900);
$table->addCell(9000, $styleCellx)->addText($texto18, $fontStylex); 
...
// Add cells
$table->addCell(2500, $styleCell)->addText('INFORMACION DE EJECUCION Y PAGOS', $fontStyle);
$table->addCell(2500, $styleCell)->addText('VALOR PAGADO', $fontStyle);
$table->addCell(2500, $styleCell)->addText('FECHA', $fontStyle);
$table->addCell(2500, $styleCell)->addText('SALDO POR PAGAR', $fontStyle);

//for($i = 1; $i <= 10; $i++) {
while ($row_rspagos = mysql_fetch_array($rspagos)){
	$table->addRow();
	$table->addCell(2500)->addText(utf8_encode($row_rspagos['ctrlpagos_desc']));
	$table->addCell(2500)->addText(number_format($row_rspagos['ctrlpagos_valor'],2,",","."));
	$table->addCell(2500)->addText($row_rspagos['ctrlpagos_fecha']);
//}
}
	$table->addCell(2500)->addText($saldo);
...
$html_dom2->load('<html><body>'.$var_infactividades.$texto20.$texto21.$texto22.$texto23.$texto24.$texto25.$texto26.$texto27.'</body></html>');
...
// Add cells
$table->addCell(2500, $styleCell)->addText('TITULO DEL ANEXO', $fontStyle);
$table->addCell(2500, $styleCell)->addText('NOMBRE DEL ANEXO', $fontStyle);


//for($i = 1; $i <= 10; $i++) {
while ($row_rsinfanexos = mysql_fetch_array($rsinfanexos)){
	$table->addRow();
	$table->addCell(2500)->addText(utf8_encode($row_rsinfanexos['anexo_titulo']));
