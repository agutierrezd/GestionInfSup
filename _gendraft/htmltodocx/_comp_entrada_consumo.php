...
$query_rscea = sprintf("SELECT * FROM q_almovconsdia WHERE doclasedoc_id_fk = %s", GetSQLValueString($colname_rscea, "int"));
...
mysql_select_db($database_oConnAlmacen, $oConnAlmacen);
$query_rssuma = sprintf("SELECT * FROM q_almovconsdia_suma WHERE doclasedoc_id_fk = %s", GetSQLValueString($colname_rssuma, "int"));
...
mysql_select_db($database_oConnAlmacen, $oConnAlmacen);
$query_rsresumen = sprintf("SELECT * FROM q_almovconsdia_resumen WHERE doclasedoc_id_fk = %s", GetSQLValueString($colname_rsresumen, "int"));
...
$texto3 = utf8_encode("FECHA DEL COMPROBANTE:<strong> ".$row_rsgedocumentos['do_fechadoc']."</strong>&nbsp;NÚMERO DE COMPROBANTE:<strong> ".$row_rsgedocumentos['do_nrodoc']."</strong><br />");
$texto3a = utf8_encode("ASIENTO Nro.:<strong> ".$row_rsalmovtodia['mdnroasiento']."</strong>&nbsp;FECHA DEL ASIENTO:<strong> ".$row_rsalmovtodia['mdfechadoc']."</strong><br />");
...
$html_dom->load('<html><body>'.$texto2.$texto3.$texto3a.$texto4.$texto5.$texto6.$texto8.$texto9.$texto10.'</body></html>');
//Fin Objeto HTML
...
$table->addCell(4500)->addImage('sns.png');
...
while ($row_rscea = mysql_fetch_array($rscea)){
	$table->addRow();
	$table->addCell(2500)->addText(utf8_encode($row_rscea['mcdcuenta']));
	$table->addCell(2500)->addText(utf8_encode($row_rscea['mcdcodelem']));
	$table->addCell(2500)->addText(utf8_encode($row_rscea['ec_nomelemento']));
	$table->addCell(2500)->addText(utf8_encode($row_rscea['um_nomunimed']));
	$table->addCell(2500)->addText(utf8_encode($row_rscea['mcd_cantmovto']));
	$table->addCell(2500)->addText(number_format($row_rscea['mcd_valunitant'],2,",","."));
	$table->addCell(2500)->addText(number_format($row_rscea['mcd_valmovto'],2,",","."));
//}
}
...
// Add cells
$table->addCell(2500, $styleCellD)->addText('', $fontStyleD);
$table->addCell(2500, $styleCellD)->addText('', $fontStyleD);
...
while ($row_rsresumen = mysql_fetch_array($rsresumen)){
	$table->addRow();
	$table->addCell(2500)->addText(utf8_encode($row_rsresumen['mcdcuenta']));
	$table->addCell(2500)->addText(utf8_encode($row_rsresumen['ca_nomcuenta']));
	$table->addCell(2500)->addText(utf8_encode($row_rsresumen['QTYITEMS']));
	$table->addCell(2500)->addText(utf8_encode(number_format($row_rsresumen['TOTALDOC'],2,',','.')));
//}
}
