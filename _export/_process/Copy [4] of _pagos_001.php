...
mysql_select_db($database_oConnContratos, $oConnContratos);
$query_rssupervisor = sprintf("SELECT * FROM q_global_supervisores WHERE id_cont_fk = %s", GetSQLValueString($colname_rssupervisor, "int"));
...
$objPHPExcel->getActiveSheet()->setCellValue('A5', utf8_encode('NOMBRE/RAZON: '.$row_rssupervisor['contractor_name'].'  NIT/DOC: '.$row_rssupervisor['cont_nit_contra_ta']));
$objPHPExcel->getActiveSheet()->setCellValue('A6', utf8_encode('VALOR: '.number_format($row_rsinfocontrato['VALORI'],2,',','.').'  PAGADO: '.number_format($row_rscontrolpagado['valorpagado'],2,',','.').'  SALDO: '.number_format(($row_rsinfocontrato['VALORI']-$row_rscontrolpagado['valorpagado']),2,',','.')));
...
$objPHPExcel->getActiveSheet()->getStyle('A1:D6')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$objPHPExcel->getActiveSheet()->duplicateStyleArray($title3, 'A7:D7');
$objPHPExcel->getActiveSheet()->setCellValue('A7', 'CONSECUTIVO  ');
$objPHPExcel->getActiveSheet()->setCellValue('B7', 'FECHA DE REGISTRO  ');
$objPHPExcel->getActiveSheet()->setCellValue('C7', 'DESCRIPCION / CONCEPTO  ');
$objPHPExcel->getActiveSheet()->setCellValue('D7', 'VALOR PAGADO  ');
