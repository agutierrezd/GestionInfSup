...
$colname_rsRZones = "-1";
if (isset($_GET['pbl_area'])) {
  $colname_rsRZones = $_GET['pbl_area'];
}
$colano_rsRZones = "-1";
if (isset($_GET['year'])) {
...
$query_rsRZones = sprintf("SELECT * FROM q_ranking_zone WHERE pbl_area = %s AND reg_ano = %s ORDER BY q_ranking_zone.TOTALR DESC", GetSQLValueString($colname_rsRZones, "text"), GetSQLValueString($colano_rsRZones, "text"));
...
$objPHPExcel->getActiveSheet()->setCellValue('A1', 'Puesto')->getColumnDimension('A')->setAutoSize(true);$objPHPExcel->getActiveSheet()->setCellValue('B1', 'PBL')->getColumnDimension('B')->setAutoSize(true);
$objPHPExcel->getActiveSheet()->setCellValue('C1', 'Area')->getColumnDimension('C')->setAutoSize(true);
$objPHPExcel->getActiveSheet()->setCellValue('D1', 'Camara Escondida (40%)  ')->getColumnDimension('D')->setAutoSize(true);
$objPHPExcel->getActiveSheet()->setCellValue('E1', 'Evaluacion V.S. (20%)  ')->getColumnDimension('E')->setAutoSize(true);
$objPHPExcel->getActiveSheet()->setCellValue('F1', 'Sondeo a Clientes (20%)  ')->getColumnDimension('F')->setAutoSize(true);
$objPHPExcel->getActiveSheet()->setCellValue('G1', 'Evaluacion de Imagen (20%)  ')->getColumnDimension('G')->setAutoSize(true);
$objPHPExcel->getActiveSheet()->setCellValue('H1', 'Total (100%)  ')->getColumnDimension('H')->setAutoSize(true);
...
$objPHPExcel->getActiveSheet()->setCellValue('B'.$xlsRow, $row_rsRZones['pbl_name'])->getStyle('B2:B'.$xlsRow)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$objPHPExcel->getActiveSheet()->setCellValue('C'.$xlsRow, $row_rsRZones['pbl_area'])->getStyle('C2:C'.$xlsRow)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$objPHPExcel->getActiveSheet()->setCellValue('D'.$xlsRow, $row_rsRZones['PESO_CE'])->getStyle('D2:D'.$xlsRow)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$objPHPExcel->getActiveSheet()->setCellValue('E'.$xlsRow, $row_rsRZones['PESO_EVS'])->getStyle('E2:E'.$xlsRow)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$objPHPExcel->getActiveSheet()->setCellValue('F'.$xlsRow, $row_rsRZones['PESO_SC'])->getStyle('F2:F'.$xlsRow)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$objPHPExcel->getActiveSheet()->setCellValue('G'.$xlsRow, $row_rsRZones['PESO_REx'])->getStyle('G2:G'.$xlsRow)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$objPHPExcel->getActiveSheet()->setCellValue('H'.$xlsRow, $row_rsRZones['TOTALR'])->getStyle('H2:H'.$xlsRow)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
...
// Rename sheet
$objPHPExcel->getActiveSheet()->setTitle('AREA_DETALLADO');
...
header('Content-Disposition: attachment;filename='.$vQualifier.'_Informe_detallado_por_area.xls"');
