...
$query_rspbl = sprintf("SELECT * FROM q_ranking_tm2 WHERE q_ranking_tm2.reg_ano = %s ORDER BY TOTALRANK DESC", GetSQLValueString($colano_rspbl, "text"));
...
$objPHPExcel->getActiveSheet()->setCellValue('H'.$xlsRow, $row_rspbl['reg_ano'])->getStyle('H2:H'.$xlsRow)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
...
// Rename sheet
$objPHPExcel->getActiveSheet()->setTitle('Ranking_anual');
...
header('Content-Disposition: attachment;filename='.$vQualifier.'_RANKING_TM_AÑO.xls');
