...
$colname_rsRZones = "-1";
if (isset($_GET['paisp'])) {
  $colname_rsRZones = $_GET['paisp'];
}
$colano_rsRZones = "-1";
if (isset($_GET['yearp'])) {
  $colano_rsRZones = $_GET['yearp'];
}
...
$query_rsRZones = sprintf("SELECT * FROM q_ranking_pais WHERE pbl_pais = %s AND reg_ano = %s ", GetSQLValueString($colname_rsRZones, "text"),GetSQLValueString($colano_rsRZones, "text"));
...
// Add some data
$objPHPExcel->getActiveSheet()->duplicateStyleArray($title, 'A1:I1');
$objPHPExcel->setActiveSheetIndex(0);
$objPHPExcel->getActiveSheet()->setCellValue('A1', 'ID')->getColumnDimension('A')->setAutoSize(true);
$objPHPExcel->getActiveSheet()->setCellValue('B1', 'PAIS')->getColumnDimension('B')->setAutoSize(true);
$objPHPExcel->getActiveSheet()->setCellValue('C1', 'TRIMESTRE')->getColumnDimension('C')->setAutoSize(true);
$objPHPExcel->getActiveSheet()->setCellValue('D1', 'CANTIDAD')->getColumnDimension('D')->setAutoSize(true);
$objPHPExcel->getActiveSheet()->setCellValue('E1', 'Camara Escondida (40%)  ')->getColumnDimension('E')->setAutoSize(true);
$objPHPExcel->getActiveSheet()->setCellValue('F1', 'Evaluacion V.S. (20%)  ')->getColumnDimension('F')->setAutoSize(true);
$objPHPExcel->getActiveSheet()->setCellValue('G1', 'Sondeo a Clientes (20%)  ')->getColumnDimension('G')->setAutoSize(true);
$objPHPExcel->getActiveSheet()->setCellValue('H1', 'Evaluacion de Imagen (20%)  ')->getColumnDimension('H')->setAutoSize(true);
$objPHPExcel->getActiveSheet()->setCellValue('I1', 'Total (100%)  ')->getColumnDimension('I')->setAutoSize(true);
...
$objPHPExcel->getActiveSheet()->setCellValue('C'.$xlsRow, $row_rsRZones['PERIODO'])->getStyle('C2:C'.$xlsRow)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$objPHPExcel->getActiveSheet()->setCellValue('D'.$xlsRow, $row_rsRZones['REGIONQ'])->getStyle('D2:D'.$xlsRow)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$objPHPExcel->getActiveSheet()->setCellValue('E'.$xlsRow, $row_rsRZones['PROMCE'])->getStyle('E2:E'.$xlsRow)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$objPHPExcel->getActiveSheet()->setCellValue('F'.$xlsRow, $row_rsRZones['PROMEVS'])->getStyle('F2:F'.$xlsRow)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$objPHPExcel->getActiveSheet()->setCellValue('G'.$xlsRow, $row_rsRZones['PROMSC'])->getStyle('G2:G'.$xlsRow)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$objPHPExcel->getActiveSheet()->setCellValue('H'.$xlsRow, $row_rsRZones['PROMREX'])->getStyle('H2:H'.$xlsRow)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$objPHPExcel->getActiveSheet()->setCellValue('I'.$xlsRow, $row_rsRZones['PROMTOTAL'])->getStyle('I2:I'.$xlsRow)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
...
// Rename sheet
$objPHPExcel->getActiveSheet()->setTitle('Ranking region');
...
header('Content-Disposition: attachment;filename='.$vQualifier.'_Ranking_region.xls');
